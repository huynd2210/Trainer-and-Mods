...
$output = Join-Path $PSScriptRoot 'FallenAces.EnhancedThrowing.dll'
& dotnet $compiler /nologo /nostdlib+ /target:library /optimize+ /langversion:latest "/out:$output" @references "$PSScriptRoot\ThrowEnhancementPlugin.cs"
if ($LASTEXITCODE -ne 0) { throw 'Plugin compilation failed.' }
if (!$NoInstall) {
    Copy-Item -LiteralPath $output -Destination (Join-Path $gameRoot 'BepInEx\plugins\FallenAces.EnhancedThrowing.dll') -Force
}
