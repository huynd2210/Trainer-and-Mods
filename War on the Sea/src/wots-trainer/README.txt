...
---------------------
F1 and F2 are defaults only. First run writes
BepInEx\config\com.wots.trainer.cfg with:

  [Controls]
  InfiniteCommandPointsKey   default F1
  RevealKey                  default F2

  [General]
  ShowStatusLine             default true

Rebind them if something else wants those keys, or if you are running the other
War on the Sea mods and would rather group the hotkeys.
