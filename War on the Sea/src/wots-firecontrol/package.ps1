# Fire Control packaging: builds the plugin, runs the AA and solution logic study, verifies
...
#   release\WoTSFireControl.zip       (plugin only, for users who already have BepInEx)
#   release\WoTSFireControl-Pack.zip  (BepInEx 5.4.23.2 x64 + plugin, self-contained)
...
$pluginDll  = Join-Path $out "WoTSFireControl.dll"
...
# The AA defaults come out of this harness rather than out of intuition, so it runs before
# anything ships and its numbers are what README.txt quotes.
Write-Host "`n== Study the AA geometry and solution convergence ==" -ForegroundColor Cyan
Push-Location (Join-Path $root "logic")
try {
    & dotnet run --project Logic.csproj -v minimal | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "Logic harness failed ($LASTEXITCODE)" }
} finally { Pop-Location }
...
if ($running.Count -gt 0) {
    ...
    Copy-Item $pluginDll (Join-Path $pluginsDir "WoTSFireControl.dll") -Force
...
Copy-Item $pluginDll (Join-Path $pluginInPack "WoTSFireControl.dll") -Force
...
Copy-Item $pluginDll (Join-Path $modPlugins "WoTSFireControl.dll") -Force
Copy-Item (Join-Path $root "README.txt") (Join-Path $stageMod "README.txt") -Force

New-Item -ItemType Directory -Path $release -Force | Out-Null
Compress-Archive -Path (Join-Path $stagePack "*") -DestinationPath (Join-Path $release "WoTSFireControl-Pack.zip") -Force
Compress-Archive -Path (Join-Path $stageMod "*")  -DestinationPath (Join-Path $release "WoTSFireControl.zip") -Force
