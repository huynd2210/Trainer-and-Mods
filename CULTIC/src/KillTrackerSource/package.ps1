# Builds and packages the CULTIC Kill Tracker BepInEx plugin.
# Produces:
#   KillTracker.zip       - the mod alone (BepInEx\plugins\KillTrackerPlugin.dll + README)
#   KillTracker-Pack.zip  - BepInEx + the mod, self-contained (extract into game folder)
#
...
$game      = Resolve-Path ".."      # CULTIC game folder (this script lives in <game>\KillTrackerSource)
$src       = $PSScriptRoot
...
if (-not $SkipBuild) {
    Write-Host "Compiling KillTrackerPlugin.dll..."
    ...
    & $csc /nologo /target:library "/out:$src\KillTrackerPlugin.dll" @refs "$src\KillTrackerPlugin.cs"
    if ($LASTEXITCODE -ne 0) { throw "Compile failed (exit $LASTEXITCODE)" }
    Copy-Item "$src\KillTrackerPlugin.dll" $plugins -Force
    Write-Host "Installed to BepInEx\plugins\KillTrackerPlugin.dll"
}

# ---- KillTracker.zip : mod only ---------------------------------------------
$stageMod = Join-Path $env:TEMP "cultic_killtracker_mod"
...
Copy-Item "$src\KillTrackerPlugin.dll" "$stageMod\BepInEx\plugins\" -Force
Copy-Item "$src\README.txt" "$stageMod\" -Force
New-Zip $stageMod (Join-Path $release "KillTracker.zip")

# ---- KillTracker-Pack.zip : BepInEx + mod, self-contained --------------------
$stagePack = Join-Path $env:TEMP "cultic_killtracker_pack"
...
Copy-Item "$src\KillTrackerPlugin.dll" "$stagePack\BepInEx\plugins\" -Force
...
@"
KillTracker-Pack - CULTIC Kill Tracker, self-contained (BepInEx + the mod)
=====================================================================
...
F7 shows/hides the kill count panel, Shift+F7 resets the level and session
columns, Ctrl+F7 exports the whole table to KillTrackerExports\*.txt.
All-time totals live in BepInEx\config\CulticKillTracker.stats.tsv.
See README.txt for details and the config file
(BepInEx\config\local.codex.cultickilltracker.cfg).
"@ | Set-Content "$stagePack\INSTALL.txt" -Encoding utf8
New-Zip $stagePack (Join-Path $release "KillTracker-Pack.zip")
